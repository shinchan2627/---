...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("𝙔𝙤𝙪 𝙖𝙧𝙚 𝙪𝙨𝙞𝙣𝙜 𝙤𝙣𝙚 𝙤𝙛 𝙢𝙮 𝙤𝙡𝙙 𝙢𝙚𝙨𝙨𝙖𝙜𝙚𝙨, 𝙥𝙡𝙚𝙖𝙨𝙚 𝙨𝙚𝙣𝙙 𝙩𝙝𝙚 𝙧𝙚𝙦𝙪𝙚𝙨𝙩 𝙖𝙜𝙖𝙞𝙣.", show_alert=True)
        return
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/tamcinemasdubt')
        ]
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        return await query.answer('⋆ 🎀  ⓟ𝓵𝒆𝐀ร乇 𝓼𝕙𝔞𝐑Ⓔ ᗩ𝓃𝐝 丂𝕦𝐏ℙ𝕆Ｒ丅  🎀  ⋆')
    ...
    if query.data.startswith("file"):
        ...
        if not files_:
            return await query.answer('𝙉𝙤 𝙨𝙪𝙘𝙝 𝙛𝙞𝙡𝙚 𝙚𝙭𝙞𝙨𝙩. ♥')
        files = files_[0]
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("𝗜 𝗟𝗶𝗸𝗲 𝗬𝗼𝘂𝗿 𝗦𝗺𝗮𝗿𝘁𝗻𝗲𝘀𝘀, 𝗕𝘂𝘁 𝗗𝗼𝗻'𝘁 𝗕𝗲 𝗢𝘃𝗲𝗿𝘀𝗺𝗮𝗿𝘁 𝗢𝗸𝗮𝘆 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        await query.answer('⋆ 🎀  ⓟ𝓵𝒆𝐀ร乇 𝓼𝕙𝔞𝐑Ⓔ ᗩ𝓃𝐝 丂𝕦𝐏ℙ𝕆Ｒ丅  🎀  ⋆')
    elif query.data == "help":
        buttons = [[
            InlineKeyboardButton('🎀 ᴍᴀɴᴜᴀʟ ꜰɪʟᴛᴇʀ', callback_data='manuelfilter'),
            InlineKeyboardButton('ᴀᴜᴛ🍬 ꜰɪʟᴛᴇʀ', callback_data='autofilter')
        ], [
            InlineKeyboardButton('ᴄ🍑ɴɴᴇᴄᴛɪ❤ɴ', callback_data='coct'),
            InlineKeyboardButton('ᴇҳᴛʀᴀ ᴍ💞ᴅꜱ', callback_data='extra')
        ], [
            InlineKeyboardButton('🏠 ʜ💞ᴍᴇ 🏠', callback_data='start'),
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("𝙄 𝙘𝙤𝙪𝙡𝙙𝙣'𝙩 𝙛𝙞𝙣𝙙 𝙖𝙣𝙮𝙩𝙝𝙞𝙣𝙜 𝙧𝙚𝙡𝙖𝙩𝙚𝙙 𝙩𝙤 𝙩𝙝𝙖𝙩. 𝘾𝙝𝙚𝙘𝙠 𝙮𝙤𝙪𝙧 𝙨𝙥𝙚𝙡𝙡𝙞𝙣𝙜")
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply("𝙄 𝙘𝙤𝙪𝙡𝙙𝙣'𝙩 𝙛𝙞𝙣𝙙 𝙖𝙣𝙮𝙩𝙝𝙞𝙣𝙜 𝙧𝙚𝙡𝙖𝙩𝙚𝙙 𝙩𝙤 𝙩𝙝𝙖𝙩 \n𝘿𝙞𝙙 𝙮𝙤𝙪 𝙢𝙚𝙖𝙣 𝙖𝙣𝙮 𝙤𝙣𝙚 𝙤𝙛 𝙩𝙝𝙚𝙨𝙚?",
                    reply_markup=InlineKeyboardMarkup(btn))
